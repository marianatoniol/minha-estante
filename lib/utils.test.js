...
import { getGradient, getSimilarity, filterBooks, filterByExplore, buildRecommendations, normalizeBookRow, parseAIJson, safeParseAIJson, COVER_GRADIENTS } from './utils.js'
...
describe('filterBooks', () => {
  ...
  it('não lança erro ao buscar em livro sem tropes nem genres', () => {
    const partial = [{ title: 'Sem Tags', authors: ['Autor'], status: 'lido' }]
    expect(() => filterBooks(partial, { search: 'algo' })).not.toThrow()
    expect(filterBooks(partial, { search: 'sem' })).toHaveLength(1)
    expect(filterBooks(partial, { search: 'algo' })).toHaveLength(0)
  })
})
...
describe('filterByExplore', () => {
  ...
  it('não lança erro com livro sem genres nem tropes', () => {
    const partial = [{ title: 'Sem Tags' }]
    expect(filterByExplore(partial, { selectedGenre: 'fantasia' })).toHaveLength(0)
    expect(filterByExplore(partial, { selectedTropes: ['distopia'] })).toHaveLength(0)
    expect(filterByExplore(partial)).toHaveLength(1)
  })
})
...
describe('buildRecommendations', () => {
  ...
  it('define basedOn como o livro origem da recomendação', () => {
    const result = buildRecommendations(recoBooks, 0)
    const recB = result.find(r => r.book.id === 2)
    expect(recB).toBeDefined()
    expect(recB.basedOn).toBeDefined()
    expect(recB.basedOn.id).not.toBe(recB.book.id)
  })

  it('atualiza basedOn e similarity quando encontra par com similaridade maior', () => {
    // A→B: inter={a,b}, union={a,b,c,d,e} → 40%
    // C→B: inter={a,b}, union={a,b,c}     → 67%  ← deve vencer
    const bs = [
      { id: 1, tropes: ['a', 'b', 'c', 'd', 'e'] },
      { id: 2, tropes: ['a', 'b'] },
      { id: 3, tropes: ['a', 'b', 'c'] },
    ]
    const result = buildRecommendations(bs, 30)
    const recB = result.find(r => r.book.id === 2)
    expect(recB.similarity).toBe(67)
    expect(recB.basedOn.id).toBe(3)
  })
})
...
describe('normalizeBookRow', () => {
  ...
  it('aplica defaults para campos ausentes dentro de books parcialmente preenchido', () => {
    const row = { id: 'z', status: 'quero ler', rating: 3, added_at: '2024-06-01', books: { title: 'Parcial' } }
    const result = normalizeBookRow(row)
...
describe('safeParseAIJson', () => {
  const fallback = { canonical_key: '', genres: [], tropes: [], summary: '' }

  it('faz parse de JSON limpo', () => {
    expect(safeParseAIJson('{"ok":true}')).toEqual({ ok: true })
  })
  ...
  it('extrai JSON quando há texto em prosa antes', () => {
    const text = 'O livro é muito bom. {"canonical_key":"tolkien_lotr","genres":["fantasia"],"tropes":["fantasia epica"],"summary":"resumo"}'
    expect(safeParseAIJson(text)).toEqual({
      canonical_key: 'tolkien_lotr',
      genres: ['fantasia'],
      tropes: ['fantasia epica'],
      summary: 'resumo',
    })
  })

  it('extrai JSON quando há texto em prosa depois', () => {
    const text = '{"ok":true} Espero que isso ajude!'
    expect(safeParseAIJson(text)).toEqual({ ok: true })
  })

  it('extrai JSON quando há texto antes e depois', () => {
    const text = 'Aqui está a resposta: {"genres":["romance"]} Qualquer dúvida é só perguntar.'
    expect(safeParseAIJson(text)).toEqual({ genres: ['romance'] })
  })

  it('retorna fallback para texto sem nenhum JSON', () => {
    expect(safeParseAIJson('não é json de jeito nenhum', fallback)).toEqual(fallback)
  })

  it('retorna fallback para string vazia', () => {
    expect(safeParseAIJson('', fallback)).toEqual(fallback)
  })

  it('retorna fallback quando o bloco { } extraído é JSON inválido', () => {
    expect(safeParseAIJson('aqui temos { chave sem valor }', fallback)).toEqual(fallback)
  })

  it('retorna {} como fallback padrão quando nenhum fallback é fornecido', () => {
    expect(safeParseAIJson('texto sem json')).toEqual({})
  })
})
